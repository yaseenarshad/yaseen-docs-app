...
describe('registerStateIpc', () => {
  ...
  it('state:set-folder checks the root and the patch shape', async () => {
    ...
    expect(store.get().folders['/v']).toEqual({ expanded: ['/v/sub'], lastFile: '/v/a.md', folds: {}, baseGroups: {}, topicsExpanded: [], favorites: [] })
    ...
    expect(store.get().folders['/v']).toEqual({ expanded: ['/v/sub'], lastFile: null, folds: {}, baseGroups: {}, topicsExpanded: [], favorites: [] })
    ...
    expect(store.get().folders['/v'].topicsExpanded).toEqual(['/v/Metrics.md'])
    // The Favorites tab's pinned paths ride the same patch (YAZ-1766 D2), checked like `expanded`.
    expect(await registered(CH.stateSetFolder)({ sender }, '/v', { favorites: ['/v/a.md', '/v/sub'] })).toEqual(ok(undefined))
    expect(store.get().folders['/v'].favorites).toEqual(['/v/a.md', '/v/sub'])
    expect(await registered(CH.stateSetFolder)({ sender }, '/v', { favorites: 'nope' })).toEqual(bad('BAD_REQUEST'))
    expect(await registered(CH.stateSetFolder)({ sender }, '/v', { favorites: [1] })).toEqual(bad('BAD_REQUEST'))
    expect(store.get().folders['/v'].favorites).toEqual(['/v/a.md', '/v/sub'])
    ...
    expect(store.get().folders['/v']).toEqual({ expanded: ['/v/sub'], lastFile: null, folds: {}, baseGroups: {}, topicsExpanded: ['/v/Metrics.md'], favorites: ['/v/a.md', '/v/sub'] })
  })
