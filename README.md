...
- **Images**: `![alt](src)` renders from the vault (Obsidian's `![alt|400](src)` width included), a pasted or dropped image is saved to `assets/images/`, and dragging a handle resizes it. Double-click opens it full-size in a lightbox, which pages through every image on the page in document order — `←` / `→` or the arrows at the bottom, with an `n / m` counter between them, stopping at either end; a page with a single image shows no arrows.
