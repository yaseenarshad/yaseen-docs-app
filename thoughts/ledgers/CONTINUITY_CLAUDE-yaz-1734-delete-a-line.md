...
- **D2 Only the head moves, one whole line per press (amended live demo 2026-09-20: mid-line no longer falls through).** HEAD on the start/end of its textblock: `⇧↓`/`⇧↑` move it to the SAME edge of the next/previous VISIBLE textblock; anchor never moves. Head MID-LINE: `⇧↓` first selects to the END of its own line, `⇧↑` to the START, then the next press continues one whole line per press. Empty block: `⇧↓` takes start, `⇧↑` takes end. Native pixel-column move only for a node selection or an `hr` in the gap; a document/zoom edge is a consumed no-op. One file `client/src/editor/lineSelection.ts` (`lineKeymap`), registered BEFORE `outlinerKeymap` in `createCrepe()` (D7) so all three editors get it. An ordinary range (no hidden line inside) deletes through ProseMirror's own path; a fold-spanning range through `deleteVisible` (D6).
- **D3 A deleted parent bullet's children survive**, joining up one level (`liftHeadlessItems`, path-agnostic). Subtree delete = a different explicit gesture, out of scope, no Future issue.
...
- Done:
  ...
  - [x] 4A audit done (verdicts in the session scratchpad `audit.md`; every [apply] item applied in 4B).
- SHIPPED: PR #69 merged to main `439ca6b` (feature commit `f82fabb`); polish PR follows.
- Now: [→] 4B apply (YAZ-1755): audit applied on the worktree — `lineSelection.ts` simplified (no behaviour change), test helpers in `markTestKit.ts`, unit-file duplicates of the matrix removed, three matrix tests added (S15b hr two blocks away, S-F8 prefix-delete branch + caret, S-F9 Enter from a start edge), comments D-only. Uncommitted; becomes the polish PR.
- Next: /commit, push, polish PR, merge (no release) → 3B Yasin's hand pass on main if he wants one → ledger closeout, HANDOFF comment on YAZ-1734 + children, rig + worktree removed.
- Remaining:
...
## Working Set
- `client/src/editor/lineSelection.ts` (new) · `client/src/editor/lineSelection.test.ts` + `lineSelection.scenarios.test.ts` (the matrix; new) · `client/src/editor/marks/markTestKit.ts` (shared harness for all editor tests) · `client/src/editor/createCrepe.ts` (registration) · `client/src/settings/hotkeys.ts` + `hotkeys.test.ts` · `docs/CONTRACTS.md` Keyboard table · `README.md`.
- Tests: `npx vitest run client/src/editor client/src/settings` · `npm run typecheck`.
